...
/*
 * On Spectrum Next, we include a trimmed-down version of the Unicode
 * case and normalization tables. The full cgunigen.c is ~3000+ lines
 * of generated data. For Spectrum Next, we use a minimal ASCII-only
 * table that preserves the same API but saves significant memory.
 *
 * On Linux, we include the full cgunigen.c for complete Unicode support.
 */
