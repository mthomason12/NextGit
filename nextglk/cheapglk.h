...
/* cheapglk.h: Private header file for Cheapass Implementation of the 
        Glk API.
    CheapGlk Library: version 1.0.7.
    Glk API which this implements: version 0.7.6.
    Designed by Andrew Plotkin <erkyrath@eblong.com>
    http://eblong.com/zarf/glk/index.html

    Adapted for NextGlk (ZX Spectrum Next + Linux dual target).
*/

#define LIBRARY_VERSION "1.0.7"
...
/* Platform-specific buffer sizes.
   Spectrum Next has ~1.7 MB total, so we reduce buffer sizes. */
#ifdef __SPECTRUM_NEXT__
#define GLI_BUF_LEN 128
#define GLI_TEMP_LEN 64
#else
#define GLI_BUF_LEN 256
#define GLI_TEMP_LEN 256
#endif
...
#if GIDEBUG_LIBRARY_SUPPORT && !defined(__SPECTRUM_NEXT__)
/* Has the user requested debug support?
   Debug console is not available on Spectrum Next. */
